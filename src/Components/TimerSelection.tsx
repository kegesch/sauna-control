...
export default class TimerSelection extends React.Component<
  ITimerSelectionProps,
  ITimerSelectionState
> {
  private timer: Timer;
  ...
  public render() {
    const timerSymbols = this.props.values.map(
      (value: number, index: number) => {
        let color = MaterialColors.white;
        if (index === this.state.selectedTimerIndex) {
    ...
    return (
      <div className={this.props.className}>
        <LayerIcon
          names={["circle", "timer"]}
          colors={[this.props.color, MaterialColors.white]}
          size={this.props.size}
        />
        {timerSymbols}
      </div>
  ...
  private onTimerEnd() {
    console.log(
      "timer of " +
        this.props.values[this.state.selectedTimerIndex] +
        "min has ended."
    );
    this.setState({ selectedTimerIndex: -1 });
    this.props.onEnd();
