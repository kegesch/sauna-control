...
export default class SensorScreen extends React.Component<ISensorScreenProperties, {}> {
    ...
    public componentWillMount(): void {
        ...
        this.subscription = upDown$.subscribe({
            error: (err) => {console.error("Error on retrieving next: " + err); },
            next: (value) => {
                if (value === "up") {
                    this.props.sensorStore.setSetPoint(this.props.sensorStore.setPoint + 1);
                } else if (value === "down") {
                    this.props.sensorStore.setSetPoint(this.props.sensorStore.setPoint - 1);
                }
            },
    ...
    public render() {
        return <div className={this.props.className}>
            <div
                onClick={() => {this._selectSetPoint("temp"); }}>
                <SectionHeader label="Temperatur" value={this.props.sensorStore.currentTemp + ""} unit="°C" />
                <BigInfo
                    selected={this.props.sensorStore.selectedSetPoint === "temp"}
                >{this.props.sensorStore.setPointTemp}</BigInfo>
            </div>
            <div
                onClick={() => {this._selectSetPoint("humid"); }}>
                <SectionHeader label="Feuchtigkeit" value={this.props.sensorStore.currentHumid + ""} unit="%" />
                <BigInfo
                    selected={this.props.sensorStore.selectedSetPoint === "humid"}
                >{this.props.sensorStore.setPointHumid}</BigInfo>
            </div>
        </div>;
    }
